...
import {
  LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ReferenceLine, ResponsiveContainer, Area
} from 'recharts';
    ...
    return (
      <div className="bg-zinc-900 border border-zinc-700 rounded-lg px-4 py-2 shadow-lg text-xs text-zinc-100">
        ...
        <div className="flex items-center gap-2">
          <span className="text-green-400 font-bold">Sharpe Ratio:</span>
          <span>{payload[0].value.toFixed(3)}</span>
        </div>
      </div>
    );
  }
...
const ResultsChart: React.FC<ResultsChartProps> = ({ data }) => {
  ...
  const testType = data.testType || data.analytics?.test_type || 'classical';
  const title = getTestTypeLabel(testType);

  return (
    <div className="w-full h-full flex flex-col items-center justify-center">
      {/* Centered Title */}
      <div className="w-full flex flex-col items-center justify-center mb-1 mt-2">
        <div className="text-2xl md:text-3xl font-extrabold text-zinc-900 dark:text-zinc-100 tracking-tight leading-tight mb-1 text-center" style={{letterSpacing: '-0.01em'}}>
          {title}
        </div>
      </div>
      {/* Subtitle + Analytics button row, both absolutely positioned in a relative container above the chart */}
      <div className="w-full max-w-3xl relative mb-2" style={{minHeight: 40}}>
        <div className="absolute left-0 right-0 mx-auto text-base text-zinc-400 font-medium text-center" style={{top: 0}}>
        ...
        {data.analytics && (
          <button
            ...
            className="absolute right-0 top-0 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-medium transition-colors flex items-center space-x-2 shadow-md"
          >
      ...
      <div className="w-full max-w-3xl bg-white dark:bg-zinc-900 rounded-xl shadow-lg p-4 border border-zinc-200 dark:border-zinc-800">
        <ResponsiveContainer width="100%" height={380}>
          <LineChart data={data.results} margin={{ top: 24, right: 32, left: 16, bottom: 32 }}>
            ...
            {/* Y Axis */}
            <YAxis
              label={{ value: 'Sharpe Ratio', angle: -90, position: 'insideLeft', style: { fill: '#64748b', fontWeight: 600, fontSize: 14 } }}
              tick={{ fill: '#64748b', fontSize: 13 }}
              axisLine={{ stroke: '#cbd5e1' }}
              tickLine={false}
              domain={[0, 'auto']}
            ...
            />
            {/* Tooltip */}
            <Tooltip content={<CustomTooltip />} cursor={{ stroke: '#60a5fa', strokeWidth: 1, opacity: 0.15 }} />
            {/* Baseline Reference Line */}
            <ReferenceLine
              y={data.baseline_sharpe}
